...
int QT_Share_MemT<T>::creat_data(int siz)
{
    ...
    if(lhshare.isAttached())
    {
        cout << ("qt share have attach\n") <<endl;
        lhshare.detach();
    }

    if(!lhshare.create(siz))
    {
        printf("qt share creat fail\n");
        qDebug()<<lhshare.error();

        if(lhshare.error() == 4)       //已经存在
        {
            if(!lhshare.attach())
            {
                qDebug("can't attatch qt share\n");
                return -2;
            }
            if(lhshare.isAttached())
            {
                cout << ("qt share test attach\n") <<endl;
            }
        }
        else
         return -1;
